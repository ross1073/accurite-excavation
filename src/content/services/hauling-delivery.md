---
title: "Hauling & Material Delivery in Ogden & Northern Utah"
metaTitle: "Hauling & Material Delivery — Northern Utah"
description: "Gravel, topsoil, fill dirt, sand, and boulder delivery plus debris removal and dump truck hauling across Weber and Davis counties."
keywords:
  primary: "material hauling Ogden Utah"
  secondary:
    - "gravel delivery Ogden"
    - "topsoil delivery Weber County"
    - "fill dirt delivery Utah"
    - "dump truck hauling Northern Utah"
faqs:
  - question: "How much does gravel delivery cost in Ogden?"
    answer: "Gravel delivery costs depend on the type of gravel, quantity ordered, and delivery distance from our yard. Minimum delivery is usually one dump truck load (10-14 tons). Contact us for a [free estimate](/free-estimate) based on your specific needs."
  - question: "How do I calculate how much gravel I need?"
    answer: "Measure the area in square feet and multiply by the desired depth in feet. Divide by 27 to get cubic yards, then multiply by 1.4 to convert to tons. Or just call us with your measurements and we will calculate it for you."
  - question: "What is the difference between fill dirt and topsoil?"
    answer: "Fill dirt is subsoil used to raise grade levels and fill holes. It compacts well but does not support plant growth. Topsoil is the nutrient-rich surface layer used for landscaping, gardens, and lawns."
  - question: "How much does road base delivery cost?"
    answer: "Road base pricing depends on quantity and delivery distance. It is a compactable blend of crushed rock and fines used for driveways, parking areas, and construction access roads. Contact us for a [free estimate](/free-estimate) based on your specific needs."
relatedServices:
  - "grading-land-clearing"
  - "demolition"
  - "residential-excavation"
---

## Material Hauling and Delivery in Ogden and Northern Utah

Need gravel delivered, fill dirt brought in, or a load of debris hauled off? AccuRite Excavating handles material hauling Ogden Utah homeowners and contractors need, from a single dump truck load of road base to multi-trip deliveries for large projects.

We're not just a delivery service. We're an excavation contractor with our own fleet, which means we can deliver material, grade it in place, and handle the broader site work if needed. That's useful when a delivery job turns into a bigger project.

---

## Materials We Deliver

### Gravel and Crushed Rock

Gravel is the most common material we deliver. The type matters for your application.

**Road base (3/4" minus compactable):** The standard material for driveways, parking pads, and construction access roads. It contains fines that pack down tight when compacted. Most residential driveway jobs use road base.

**3/4" clean crushed rock:** Clean rock without fines, used for drainage applications, French drains, leach fields, and areas where water needs to move through the rock layer. It doesn't compact well, which is the point.

**Pea gravel:** Small rounded stone used for walkways, playgrounds, drainage, and decorative applications. Common in Weber County landscaping.

**Rip rap and cobble:** Larger angular rock used for erosion control on slopes, stream banks, and drainage channels. Also used decoratively.

### Fill Dirt

Fill dirt is subsoil material used to raise grade elevations, fill holes, or build up areas for construction. It doesn't contain the organic material that topsoil does. Fill dirt compacts well, which makes it appropriate for subgrade applications under driveways, slabs, and foundations.

The quality of fill dirt matters. We provide clean fill, not debris-laden or contaminated material.

### Topsoil

Topsoil is what you need for lawns, gardens, and landscaping. It's the nutrient-rich upper soil layer that supports plant growth. If you're reseeding a yard, establishing a garden bed, or finishing a landscaping project, topsoil is what goes on top.

We deliver screened topsoil to Weber and Davis County addresses.

### Sand

Sand is used for leveling pavers and flagstone, playground fill, drainage applications, and mortar mix jobs. We deliver in dump truck quantities for larger projects.

### Boulders and Decorative Rock

We deliver landscape boulders, feature rocks, and decorative stone for yard and garden projects. Larger boulders may need equipment for placement, which we can coordinate.

---

## Hauling and Debris Removal

Material hauling isn't just deliveries. Sometimes you need material taken away.

### Excavation Spoils Removal

When [residential excavation](/services/residential-excavation) or [land clearing and grading](/services/grading-land-clearing) generates material that can't be reused on site, we haul it off. Excess dirt, rock, and debris from excavation projects gets loaded and removed as part of the job.

### Demolition Debris Hauling

Concrete, asphalt, old structures, and construction debris from [demolition projects](/services/demolition) need to go somewhere. We load and haul demolition debris to appropriate disposal sites.

### Yard Waste and Site Cleanup

Large landscaping projects, lot clearing, and property cleanups generate material that needs hauling. We handle truck loads of brush, soil, rock, and yard waste.

---

## Service Area

We deliver throughout Weber and Davis counties, including:

Ogden, Roy, Riverdale, South Ogden, North Ogden, Pleasant View, Harrisville, Farr West, Washington Terrace, Clearfield, Layton, Clinton, Syracuse, and surrounding communities.

Deliveries to Morgan County, Box Elder County, and Cache Valley may be available. Call to ask about delivery to your location.

---

## Calculating How Much Material You Need

The math isn't complicated, but it trips people up.

**For gravel and fill dirt:**
1. Measure the area in length x width (in feet)
2. Decide on depth (in inches), then divide by 12 to get feet
3. Multiply length x width x depth to get cubic feet
4. Divide by 27 to get cubic yards
5. Multiply cubic yards by 1.4 to get approximate tons

A 20x30 foot driveway at 4 inches deep works out to roughly 9 tons of road base. When in doubt, call us with your measurements and we'll calculate it.

**Minimum delivery:** Most deliveries are a minimum of one truck load, which is 10-14 tons depending on material density.

---

## Delivery Pricing

Delivery pricing depends on a few key variables:

- **Material type** — road base, topsoil, fill dirt, sand, and decorative rock each have different per-ton costs based on current material pricing from local quarries and suppliers
- **Quantity** — larger orders typically get better per-ton pricing
- **Delivery distance** — deliveries closer to our Ogden yard cost less than runs to the edges of Weber or Davis County

We deliver in dump truck loads (10-14 tons per load depending on material density). Call us with your material type, quantity, and delivery address and we'll give you a delivered price. [Get a free estimate](/free-estimate) for your delivery or hauling project.

---

## Why Hire AccuRite for Material Hauling

We've been operating in Northern Utah since 1995. We know where to source quality material in Weber County, and we know the roads and access conditions for properties throughout the area.

Because we're a full excavation contractor, not just a delivery service, we can do more than drop material at the curb. We can grade it in, compact it, and tie the delivery into a larger project scope if needed. That saves you the step of hiring a second contractor to spread and grade what we deliver.

---

## Schedule a Delivery or Pickup

Call us with your address, the material you need, and the quantity. We'll give you a price and schedule the delivery. For debris removal and hauling, describe what needs to go and we'll give you a hauling quote.

AccuRite Excavating serves all of Weber and Davis counties from our Ogden base.
