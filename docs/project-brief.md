...
- Monthly branded PDF SEO report, scheduled for the 1st of each month at 8am local via `launchd`, delivered as a Teamwork task assigned to Ross for review before forwarding to the client. Pipeline lives at `/Users/rosswalker/projects/3sm_code/monthly-reports/accurite/`; the task lands in project 210055 "Shawn Durrant" / tasklist Inbox (moved off the R&R project 2026-08-11). Standing rule: positive framing only — see project-scoped memory `feedback_monthly_report_positive_framing.md`. **The schedule has never yet produced a report** — a nonexistent CLI flag killed every run from 2026-06-01 to 08-01; fixed 2026-08-11, first real test is 2026-09-01. Every report so far was generated by hand. The GBP section runs on hand-entered dashboard figures plus an automatic public review pull (`pull_gbp_reviews.py`), not the Business Profile API, which Google never approved.
