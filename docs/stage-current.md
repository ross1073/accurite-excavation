...
- ~~**Push `~/.claude` commit `878d750`**~~ — **CLOSED 2026-08-11.** The hash no longer exists in the repo (rewritten or amended); `~/.claude` HEAD equals `origin/main` with a clean tree, so the content shipped under a different hash. Nothing outstanding.
