...
- **Blog indexation follow-up (2026-06-23):** Cassandra to manually Request Indexing in GSC for the 2 posts Google hasn't crawled — `/blog/excavation-safety-on-federal-projects` (was unknown to Google; its only internal link was the .md 404) and `/blog/national-park-service-project` (discovered, not crawled). Task TW #36664201, due 2026-06-26. Separately, 3 posts are "crawled, currently not indexed" (`avoid-flooding-new-home`, `retaining-wall-permit-utah`, `retaining-wall-types-utah-soil`) — Google's quality decision, not a discovery gap; fixing needs content depth, not resubmission. Not yet scoped.
